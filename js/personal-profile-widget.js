var PersonalProfileWidget;

(function($) {
	PersonalProfileWidget.updateSizeDropdownOptions = function( field, sizes ) {
		...
		if ( sizes ) {
			$.each( sizes, function( key, size ) {
				...
				if ( key in PersonalProfileWidget.imageSizeNames ) {
					name = PersonalProfileWidget.imageSizeNames[ key ];
				}
		...
		if ( ! options ) {
			name = PersonalProfileWidget.imageSizeNames['full'] || PersonalProfileWidget.fullSizeLabel;
	...
	mediaControl = {
		...
		frame: function() {
			...
			this._frame = wp.media({
				title: $control.data('title') || PersonalProfileWidget.frameTitle,
				library: {
					type: $control.data('media-type') || 'image'
				},
				button: {
					text: $control.data('update-text') || PersonalProfileWidget.frameUpdateText
				},
		...
		init: function() {
			$('#wpbody').on('click', '.personal-profile-widget-control-choose', function(e) {
				var targetSelector;

				e.preventDefault();

				$control = $(this).closest('.personal-profile-widget-control');

				targetSelector = $control.data('target') || '.personal-profile-widget-control-target';
